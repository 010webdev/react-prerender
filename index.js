...
const preloadResources = (opt) => {
    ...
    page.on("response", async (response) => {
        ...
        if (/^http:\/\/localhost/i.test(responseUrl)) {
            ...
            if (
                preloadImages &&
                /\.(png|jpg|jpeg|webp|gif|svg)$/.test(responseUrl)
            ) {
                ...
                ajaxCache[route] = await response.json()
    ...
    return page.evaluate(
        (basePath, http2PushManifest, inlineCss) => {
            ...
            const chunkScripts = localScripts.filter((x) => {
                ...
                return (
                    matched && matched[1] !== "main" && matched[1] !== "vendors"
                )
            })
            ...
            const mainScripts = localScripts.filter((x) => {
                ...
                return (
                    matched &&
                    (matched[1] === "main" || matched[1] === "vendors")
                )
            })
...
const fixWebpackChunksIssue2 = ({
    ...
    return page.evaluate(
        (basePath, http2PushManifest, inlineCss) => {
            ...
            for (
                let i = headScripts.length;
                i <= chunkScripts.length - 1;
                i++
            ) {
                const x = chunkScripts[i]
                if (x.parentElement && mainScript.parentNode) {
    ...
    await crawl({
        ...
        beforeFetch: async ({ page, route }) => {
            ...
            if (
                ...
                const { ajaxCache: ac, http2PushManifestItems: hpm } =
                    preloadResources({
                        page,
                        basePath,
                        preloadImages,
        ...
        afterFetch: async ({ page, route, browser, addToQueue }) => {
            ...
            if (options.inlineCss) {
                ...
                if (http2PushManifest) {
                    ...
                    for (
                        let i = http2PushManifestItems[route].length - 1;
                        i >= 0;
                        i--
                    ) {
